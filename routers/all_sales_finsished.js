...
router.post("/preview/:id", auth , async (req, res) => {
    try {
        ...
        const promises = new_sales.sale_product.map( async (product_details) => {
            var warehouse_data = await warehouse.findOne({ name: warehouse_name, room: product_details.room_name, warehouse_category: "Finished Goods" });
           
            const match_data = warehouse_data.product_details.map((data) => {
                
            //     console.log(data.product_name + " == " + product_details.product_name  + " && " + 
            ...
            //     data.product_stock + " <<<==>>> " +  product_details.quantity + " >>> " +product_details.room_name + 
            //     " && " + data._id + " == " + product_details.id_transaction_from
            //  )

                if (data.product_name == product_details.product_name && data.bay == product_details.bay && data.expiry_date == product_details.expiry_date  && data.production_date == product_details.production_date && data.batch_code == product_details.batch_code  && data._id == product_details.id_transaction_from) {
                    // console.log("true")
                    ...
                    if(data.product_stock > 0){
                        ...
                        if(data.out_invoice == undefined){
                            data.out_invoice  = invoice+"~"
                        }else{
                            var data_invoice = data.out_invoice.split("~");
                            var string_inv = '';
                            for (let index = 0; index <= data_invoice.length-1; index++) {
                                const element = data_invoice[index];
                                if(element.length > 0){
                                    string_inv += element+"~"
                                }
                            }
                            data.out_invoice  = string_inv
                        }
                        // var data_invoice = data.out_invoice.split("~");
                        
                        // console.log(string_inv)
        ...
        Promise.all(promises)
                .then(async (updatedWarehouseDataArray) => {
                    ...
                    try {
                        ...
                        for (n in product_list) {
                            var dataVal = "FG"
                            if(new_sales.warehouse_name == "DRY GOODS"){
                                dataVal = "DG"
                            }
                            arrayItems +=  '<tr>'+
